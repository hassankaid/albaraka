...
interface ShellProps {
  ...
  onExitDemo?: () => void;
  upstreamForced?: boolean;
  signed?: boolean;
}

export function Shell({ children, onRestart, demoLabel, onExitDemo, upstreamForced, signed }: ShellProps) {
  return (
    <div
    ...
    >
      <div className="mx-auto w-full max-w-[860px] px-4 py-8 md:py-10">
        ...
        {upstreamForced && (
          <div
            className="mb-3 rounded-xl px-4 py-2 text-[12px] leading-[1.5]"
            style={{ background: "rgba(201,138,76,0.08)", border: "0.5px solid #c98a4c", color: "#e8c9a0" }}
          >
            ⚠ <strong>Module amont forcé</strong> — une étape précédente (value ladder, offre, pricing ou garantie) a été
            validée par force. Les pré-remplissages restent dispos mais vérifie les données venant de tes modules précédents.
          </div>
        )}
        {signed && (
          <div
            className="mb-3 rounded-xl px-4 py-2 text-[12px] leading-[1.5]"
            style={{ background: "rgba(76,201,135,0.08)", border: "0.5px solid #4cc987", color: "#b0e8c5" }}
          >
            ✓ <strong>M8 signé et verrouillé</strong> — données transmises au module suivant.
          </div>
        )}
        ...
        >
          ...
          <header className="mb-5 flex items-center justify-between border-b pb-4" style={{ borderColor: "rgba(201,168,76,0.18)" }}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="flex items-center gap-2 text-[13px] font-semibold leading-tight">
                  Module 8 — PREUVE SOCIALE
                  {demoLabel && (
                    <button
                      type="button"
                      onClick={onExitDemo}
                      className="inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] font-semibold transition-colors"
                      style={{ background: "rgba(255,180,80,0.12)", border: "0.5px solid rgba(255,180,80,0.4)", color: "#FFB450" }}
                    >
                ...
                <div className="mt-0.5 text-[10px] font-medium uppercase tracking-[0.12em] text-[#C9A84C]">
                  Liberty · Récolte de témoignages
                </div>
              </div>
            </div>
            {onRestart && (
              <button
                ...
                style={{ background: "rgba(201,168,76,0.06)", border: "0.5px solid rgba(201,168,76,0.4)", color: "#C9A84C" }}
              >
