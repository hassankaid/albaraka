...
interface DemoSelectorProps {
  onBack: () => void;
  onSelect: (demoCase: M2DemoCase) => void;
}

const SEG_META: Record<M2DemoCase["segment"], { emoji: string; label: string; tag: string }> = {
  argent: { emoji: "💰", label: "Argent", tag: "Business · Liberté financière" },
  relation: { emoji: "❤️", label: "Relations", tag: "Couple · Famille · Mariage" },
  sante: { emoji: "🌱", label: "Santé", tag: "Corps · Esprit · Bien-être" },
};

export function DemoSelectorScreen({ onBack, onSelect }: DemoSelectorProps) {
  const groups = (Object.keys(SEG_META) as M2DemoCase["segment"][]).map((s) => ({
    segment: s,
    cases: M2_DEMO_CASES.filter((c) => c.segment === s),
  }));

  const readyCount = M2_DEMO_CASES.filter((c) => c.ready).length;

  function handleClick(c: M2DemoCase) {
    if (!c.ready) {
      toast.info(`« ${c.title.split("·")[0].trim()} » sera prêt prochainement.`, {
        description: "Pour l'instant, seul Karim est disponible en démo complète.",
      });
      return;
    }
    onSelect(c);
  ...
  return (
    <div>
      ...
      <p className="mb-6 max-w-[580px] text-[13px] leading-[1.6] text-white/60">
        10 cas halal pré-remplis ({readyCount} disponibles, {10 - readyCount} en production) pour
        explorer le M2 sans toucher à ton vrai parcours.{" "}
