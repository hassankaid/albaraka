...
import { MecanismeScreen } from "./screens/MecanismeScreen";
import { VehiculeScreen } from "./screens/VehiculeScreen";
import { BonusScreen } from "./screens/BonusScreen";
import { GarantieScreen } from "./screens/GarantieScreen";
...
import { usePersistedM3State } from "./lib/usePersistedState";
import { defaultM3State, type M3Step, type M3State } from "./lib/types";
import { M3_DEMO_CASES, type M3DemoCase } from "./lib/demo-cases";

export default function M3AnatomiePage() {
  const navigate = useNavigate();
  const { user, profile, isLoading: authLoading } = useAuth();
  const { hasLiberty, isLoading: passLoading } = useUserPass();
  const userId = user?.id ?? null;

  const persisted = usePersistedM3State(userId);
  const [showRestart, setShowRestart] = useState(false);
  const [showExitDemo, setShowExitDemo] = useState(false);
  const [showDemoSelector, setShowDemoSelector] = useState(false);
  const preDemoSnapshot = useRef<M3State | null>(null);
  ...
  const goTo = useCallback((step: M3Step) => {
  ...
  const handleSelectDemo = useCallback((c: M3DemoCase) => {
    if (!c.patch) return;
    if (!persisted.state.demoMode) preDemoSnapshot.current = persisted.state;
    const fresh = defaultM3State();
    const next: M3State = { ...fresh, ...c.patch, demoMode: c.key } as M3State;
  ...
  const demoLabel = persisted.state.demoMode
    ? M3_DEMO_CASES.find((c) => c.key === persisted.state.demoMode)?.title.split("·")[0].trim() ?? null
    : null;
    ...
    return (
      <div className="-mx-6 -my-6 flex min-h-[calc(100vh-4rem)] w-[calc(100%+3rem)] items-center justify-center bg-[#050505] p-6 text-center">
        <div className="max-w-md">
          ...
          <p className="text-sm text-white/60">L'outil M3 ANATOMIE est réservé aux porteurs du pass LIBERTY.</p>
        ...
        <>
          {state.current_step === "welcome" && (
            <WelcomeScreen
              state={state}
              setState={setState}
              onStart={() => goTo("promesse")}
              onOpenDemo={() => setShowDemoSelector(true)}
            />
          )}
          {state.current_step === "promesse" && (
            <PromesseScreen state={state} setState={setState} onBack={() => goTo("welcome")} onNext={() => goTo("mecanisme")} />
          )}
          {state.current_step === "mecanisme" && (
            <MecanismeScreen state={state} setState={setState} onBack={() => goTo("promesse")} onNext={() => goTo("vehicule")} />
          )}
          {state.current_step === "vehicule" && (
            <VehiculeScreen state={state} setState={setState} onBack={() => goTo("mecanisme")} onNext={() => goTo("bonus")} />
          )}
          {state.current_step === "bonus" && (
            <BonusScreen state={state} setState={setState} onBack={() => goTo("vehicule")} onNext={() => goTo("garantie")} />
          )}
          {state.current_step === "garantie" && (
            <GarantieScreen state={state} setState={setState} onBack={() => goTo("bonus")} onNext={() => goTo("urgence")} />
          )}
          {state.current_step === "urgence" && (
            <UrgenceScreen state={state} setState={setState} onBack={() => goTo("garantie")} onNext={() => goTo("prix")} />
          )}
          {state.current_step === "prix" && (
            <PrixScreen state={state} setState={setState} onBack={() => goTo("urgence")} onNext={() => goTo("lock")} />
          )}
          {state.current_step === "lock" && (
            <LockScreen state={state} setState={setState} userId={userId} onBack={() => goTo("prix")} flushNow={flushNow} />
          )}
        </>
      )}
        ...
        <AlertDialogContent style={{ background: "#0F0E0A", border: "0.5px solid rgba(201,168,76,0.4)", color: "#ECEEF4" }}>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-white/60">
              Toutes les données saisies dans le M3 seront perdues. Le contexte M1+M2 sera réimporté.
