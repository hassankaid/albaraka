...
const PRODUCT_LABEL = "PASS LIBERTY";
const COUPON_1X_ONLY = "LIBERTY2000";
 ...
 * ------------------------------------------------------------------ */
export default function LibertyCheckout() {
  ...
  const installments =
    Number.isInteger(parsedInstallments) && parsedInstallments >= 1 && parsedInstallments <= MAX_INSTALLMENTS
      ? parsedInstallments
      : 1;
...
function CheckoutForm({
  ...
  const [agreements, setAgreements] = useState<AgreementItem[]>(() => initAgreements("LIBERTY"));
  ...
  async function onApplyCoupon() {
    ...
    if (!code) return;
    // LIBERTY2000 ne s'applique qu'en paiement comptant (1x).
    if (code === COUPON_1X_ONLY && installments !== 1) {
      setCoupon({ status: "invalid", reason: "not_1x" });
      toast.error(
        `Le code ${code} n'est valable qu'en paiement comptant (1 fois). Choisis "Paiement comptant" pour l'utiliser.`,
      );
      return;
    }
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const returnUrl = `${window.location.origin}/merci-liberty${testMode ? "?test=1" : ""}`;
