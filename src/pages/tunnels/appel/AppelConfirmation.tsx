// ─────────────────────────────────────────────────────────────────────────
// Page de confirmation de la réservation d'appel (page /appel-conference).
//
// Atteinte après réservation, via la REDIRECTION Calendly (à configurer côté
// Calendly : événement « inscription-conference » → Confirmation Page →
...
export default function AppelConfirmation() {
  const data = useMemo(() => {
  ...
  return (
    <div style={{ position: "relative", minHeight: "100vh", background: T.bg, color: T.cream, fontFamily: T.body, overflowX: "hidden" }}>
      ...
      <style>{`
        @keyframes albac-rise { from{opacity:0;transform:translateY(20px)} to{opacity:1;transform:none} }
        .albac-rise { animation: albac-rise .8s cubic-bezier(.2,.7,.3,1) both; }
      `}</style>
      ...
      <main style={{ position: "relative", zIndex: 1, maxWidth: 640, margin: "0 auto", padding: "clamp(30px,6vw,56px) 22px clamp(56px,10vw,90px)", textAlign: "center" }}>
        <div className="albac-rise" style={{ width: 72, height: 72, margin: "0 auto 22px", borderRadius: "50%", border: `1px solid ${T.goldLine}`, display: "grid", placeItems: "center", background: "radial-gradient(circle, rgba(201,160,78,0.16), transparent 70%)" }}>
          <span style={{ fontSize: 38, lineHeight: 1 }}>✅</span>
        </div>

        <h1 className="albac-rise" style={{ animationDelay: "60ms", fontFamily: T.display, fontWeight: 600, fontSize: "clamp(1.9rem,6vw,3rem)", lineHeight: 1.1, color: T.cream, margin: "0 0 12px", textTransform: "uppercase" }}>
          Appel confirmé
        </h1>

        <p className="albac-rise" style={{ animationDelay: "120ms", fontFamily: T.body, fontSize: "clamp(1rem,2.7vw,1.15rem)", lineHeight: 1.55, color: T.creamMuted, margin: "0 auto 30px", maxWidth: 480 }}>
          {data.firstName ? (
            <>Bravo <strong style={{ color: T.goldBright, fontWeight: 600 }}>{data.firstName}</strong>, ton appel avec l'équipe de Sidali est réservé.</>
          ) : (
            <>Ton appel avec l'équipe de Sidali est réservé.</>
          )}{" "}
          Voici le récapitulatif.
        </p>

        {/* Récapitulatif du RDV */}
        <div className="albac-rise" style={{ animationDelay: "180ms", textAlign: "left", background: T.bgCard, border: `1px solid ${T.goldLine}`, borderRadius: 20, padding: "clamp(20px,4vw,30px)", boxShadow: "0 20px 60px rgba(0,0,0,0.4)" }}>
          {rows.length > 0 ? (
            rows.map((r, i) => (
        ...
        <p className="albac-rise" style={{ animationDelay: "260ms", fontFamily: T.body, fontSize: "0.9rem", color: T.creamMuted, marginTop: 24, lineHeight: 1.6 }}>
