...
export function DiscordGate({ onJoined }: Props) {
  ...
  // Étape 1 : l'user a cliqué sur "Accéder à Discord" et le tab a été ouvert
  // → débloque le bouton 2 "J'ai rejoint le groupe Discord".
  ...
  // Étape 1 : ouvre Discord dans un nouvel onglet (pas de redirection auto,
  // l'user revient ensuite cliquer sur le bouton de confirmation).
  function onAccessDiscordClick() {
    if (!confirmed) return;
    window.open(DISCORD_INVITE_URL, "_blank", "noopener,noreferrer");
    setDiscordOpened(true);
  }

  // Étape 2 : l'user confirme avoir rejoint Discord → on enregistre côté DB
  // et on poursuit l'onboarding.
  async function onConfirmJoinedClick() {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Étape 1 : Bouton "Accéder à Discord" — actif quand checkbox cochée */}
        <button
          type="button"
          onClick={onAccessDiscordClick}
          disabled={!confirmed}
          style={{
            width: "100%",
        ...
        {/* Étape 2 : Bouton de confirmation — actif après ouverture du tab Discord */}
        <button
          type="button"
          onClick={onConfirmJoinedClick}
          disabled={joining || !discordOpened}
          style={{
            width: "100%",
            background: !discordOpened ? "transparent" : BRAND.gold,
            color: !discordOpened ? "rgba(245,241,230,0.5)" : BRAND.black,
            border: !discordOpened ? `1px solid ${BRAND.goldSoft}` : "none",
            borderRadius: 8,
            padding: "17px 20px",
            fontSize: 14,
            fontWeight: 500,
            cursor: joining || !discordOpened ? "not-allowed" : "pointer",
            letterSpacing: 3,
            fontFamily: "inherit",
            opacity: joining || !discordOpened ? 0.5 : 1,
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            gap: 10,
        ...
        >
          {joining ? "ENREGISTREMENT..." : "J'AI REJOINT LE GROUPE DISCORD"}
        </button>
        ...
        >
          {!confirmed
            ? "Coche la case ci-dessus pour activer les boutons."
            : !discordOpened
              ? "Étape 1 : clique sur « Accéder à Discord » pour ouvrir le serveur dans un nouvel onglet."
              : "Étape 2 : une fois inscrit·e sur Discord, reviens ici et clique sur « J'ai rejoint le groupe Discord » pour continuer."}
        </div>
      </div>
    </div>
  );
