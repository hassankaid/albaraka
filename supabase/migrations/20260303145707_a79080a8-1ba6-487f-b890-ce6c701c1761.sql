...
CREATE VIEW public.leads_enriched AS
SELECT l.id,
    l.contact_id,
    l.source,
    l.source_detail,
...
FROM leads l
    ...
    LEFT JOIN LATERAL (
        SELECT calls.id, calls.contact_id, calls.lead_id, calls.calendly_event_id,
            calls.event_type, calls.scheduled_at, calls.duration_minutes, calls.assigned_to,
            calls.status, calls.outcome, calls.notes, calls.created_at, calls.updated_at,
            calls.canceled_at, calls.canceled_by, calls.cancellation_reason, calls.rescheduled_from,
            calls.raw_full_name, calls.raw_email, calls.raw_phone
        FROM calls
        WHERE calls.contact_id = l.contact_id AND calls.status = 'planifie' AND calls.scheduled_at > now()
