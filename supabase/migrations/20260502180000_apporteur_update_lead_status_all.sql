-- Élargit la whitelist de la RPC apporteur_update_lead_status à TOUS les
-- statuts (incluant pas_de_reponse et pas_de_reponse_post_conference qui
-- déclenchent un recyclage instantané).
--
-- Si le statut choisi est dans INSTANT_RECYCLE_STATUSES, la RPC fait
-- automatiquement le recyclage (désaffecte assigned_to + recycled_at = now)
-- pour garder cohérence : un lead "pas de réponse" doit sortir du pipeline
-- du commercial. L'apporteur_id reste inchangé (le lead reste visible côté
-- apporteur).
