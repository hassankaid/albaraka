...
async function ensureCustomLinkOrder(
  ...
  // On lit aussi grants_* pour pouvoir creer l'enrollment plus bas si besoin.
  const { data: linkRow } = await supabase
    .from("payment_links")
    .select("id, status, sale_id, grants_offer_id, grants_formation_ids, auto_generated")
    .eq("id", linkId)
    .maybeSingle();
  if (linkRow?.sale_id) {
  ...
  // ── Application des grants_* (perimetre d'acces) ──
  // Si le lien a des grants (formations a la carte ou pass), on cree le
  // profile (auth user + profiles) + l'enrollment ou le pass associes. Mode
  // identique a ensureBonCommandeOrder : access auto-grant + email d'acces.
  //
  // Pour un lien custom CEO classique sans grants_* (juste un encaissement),
  // ce bloc est skip → comportement actuel inchange.
  if (linkRow && (linkRow.grants_offer_id || (linkRow.grants_formation_ids && linkRow.grants_formation_ids.length > 0))) {
    try {
      await applyPaymentLinkGrants(supabase, {
        saleId: sale.id,
        email,
        fullName,
        metadata,
        grantsOfferId: linkRow.grants_offer_id,
        grantsFormationIds: linkRow.grants_formation_ids ?? [],
        autoGenerated: !!linkRow.auto_generated,
      });
    } catch (e) {
      // On ne fail PAS le webhook si l'application des grants echoue : la
      // vente est deja persistee, l'admin peut rejouer manuellement. On log
      // explicitement pour pouvoir corriger.
      console.error(
        `[custom_link/grants] applyPaymentLinkGrants failed for sale=${sale.id} link=${linkId}:`,
        e,
      );
    }
  }
...
/**
 * Applique le perimetre d'acces (grants_*) d'un payment_link apres creation
 * de la vente. Cree le profile (auth user + row profiles) si necessaire,
 * met a jour sale.buyer_profile_id, puis :
 *   - grants_formation_ids : INSERT formation_enrollments (source='a_la_carte')
 *     idempotent grace au UNIQUE INDEX (user_id, formation_id).
 *   - grants_offer_id : reserve pour Sprint H (pass AL BARAKA / Liberty via
 *     un lien custom). Pour l'instant, log seulement.
 *
 * Email d'acces : envoye via send-apporteur-access-email pour que le client
 * puisse se loguer et acceder a la formation immediatement (meme en mode
 * differe : il a achete, il a l'acces).
 */
async function applyPaymentLinkGrants(
  supabase: SupabaseClient,
  args: {
    saleId: string;
    email: string;
    ...
    grantsFormationIds: string[];
    autoGenerated: boolean;
  },
): Promise<void> {
  const { saleId, email, fullName, metadata, grantsOfferId, grantsFormationIds } = args;
  const logTag = args.autoGenerated ? "[formation_a_la_carte]" : "[custom_link/grants]";

  // ── Find or create profile (pattern bon_commande lignes 192-260) ──
  let profileId: string | null = null;
  ...
  if (existingProfile) {
    profileId = existingProfile.id;
    if (existingProfile.origin === "manual") {
      await supabase.from("profiles").update({ origin: "a_la_carte" }).eq("id", profileId);
    }
  } else {
    const { data: created, error: createErr } = await supabase.auth.admin.createUser({
      email,
    ...
    if (created?.user?.id) {
      ...
      if (!alreadyExists) {
        console.error(`${logTag} createUser failed:`, createErr);
        throw createErr ?? new Error("createUser returned no user");
      }
      // L'utilisateur existe dans auth.users mais pas dans profiles (cas rare)
      const { data: list } = await supabase.auth.admin.listUsers({ page: 1, perPage: 1000 });
      const existing = list?.users?.find(
        (u: { email?: string | null }) => (u.email || "").toLowerCase() === email,
      );
      ...
      .upsert(
        {
          ...
          country: normalizeCountryToFrName(metadata.customer_country),
          role: "apporteur",
          origin: args.autoGenerated ? "a_la_carte" : "custom_link",
        },
        { onConflict: "id" },
      );
  }
  ...
  if (!profileId) {
    throw new Error("profileId is null after find_or_create");
  }

  // ── Attache buyer_profile_id sur la vente ──
  await supabase.from("sales").update({ buyer_profile_id: profileId }).eq("id", saleId);

  // ── Grants : formations a la carte ──
  if (grantsFormationIds.length > 0) {
    const enrollmentRows = grantsFormationIds.map((formationId) => ({
      user_id: profileId,
      formation_id: formationId,
      source: "a_la_carte" as const,
      notes: `Achat via payment_link sale=${saleId}`,
    }));
    // ON CONFLICT (user_id, formation_id) DO NOTHING grace au UNIQUE INDEX existant.
    // Supabase ne supporte pas `.onConflict()` chainable mais upsert le gere.
    const { error: enrollErr } = await supabase
      .from("formation_enrollments")
      .upsert(enrollmentRows, { onConflict: "user_id,formation_id", ignoreDuplicates: true });
    if (enrollErr) {
      console.error(`${logTag} formation_enrollments insert failed:`, enrollErr);
      // On continue : la vente est OK, l'enrollment pourra etre cree manuellement.
    } else {
      console.log(
        `${logTag} granted ${grantsFormationIds.length} formation(s) to profile=${profileId}`,
      );
    }
  }
  ...
  try {
    ...
    if (!res.ok) {
      const text = await res.text();
      console.error(`${logTag} access-email failed ${res.status}: ${text}`);
    } else {
      console.log(`${logTag} access-email sent to profile=${profileId}`);
    }
  } catch (e) {
    console.error(`${logTag} failed to invoke access-email:`, e);
  }
}
